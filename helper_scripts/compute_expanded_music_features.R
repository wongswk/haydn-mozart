# This script is called immediately after the clean_music_data.R script. 
# Based on the cleaned files from that script, this script computes the features and forms one data frame per dataset, 
# where column 1 is the name of the movement, column 2 is a dummy variable of composer
# (0 for Mozart and 1 for Haydn), and the remaining columns are features. 

# The difference between this script and the compute_music_features.R script is that this one computes segment features for all 
# segment lengths m = 8, 9, 10, ..., 17, 18, as opposed to only m = 8, 10, 12, ..., 16, 18. 

# Load libraries 
library(Hmisc) # This is used for computing weighted quantiles. 
library(stringr) # for easier string manipulation

# Read in functions
source(paste0(home_path, "helper_scripts/cleaning_functions.R"))
...
end_ind=file_num*c(1:num)
begin_ind=end_ind - (file_num - 1)
...
voicepair_int_dist.out=data.frame(matrix(dist,byrow=F,nrow=length(name),ncol=ncol(pair)*12))
...
Base.df = data.frame(cbind(name, c(rep(0, Mozart_num), rep(1, Haydn_num))))
names.df=c("name", "M0")
...
if (dataset == "HM107") {
  # As noted in lines 646-651 of this script, for pre-processing we will remove 5 more median proportion of minor third relative pitch interval features in the HM107 dataset, 
  # due to problems of 0-variance and repeated 0s. 
  Feature.df = Feature.df[, -c(607, 608, 645:647)]
}

filename = paste0(output_path, "Feature_expanded_", dataset, ".csv")

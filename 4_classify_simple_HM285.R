# This script classifies the composer of each movement in the HM285 dataset using the LOO CV scheme on a reduced feature set,
# by calling the main LOO.R script with settings defined for this task. 
# Rather than using the full feature set, this script uses the reduced feature set, which contains only "simple" features (basic and interval). 
# For each CV fold, 
...
simple = T            # if T, we will use only "simple" features (basic and interval features) for feature selection and classification
